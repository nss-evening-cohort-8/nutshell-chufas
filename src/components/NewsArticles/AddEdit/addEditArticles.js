...
const buildAddForm = () => { // we need this function just to reuse fomBuilder
  ...
  domString += '<button id="add-article" class="btn-info btn">Add Article</button>';
  domString += '<button class="btn-danger cancel-form btn">Cancel</button>';
...
const cancelForm = () => {
  getArticles.getAllArticlesFromDb()
    .then(() => {
      $('#add-articles').hide();
      $('#articles').show();
      initializeArticlesPage();
...
$('body').on('click', '.cancel-form', cancelForm);
